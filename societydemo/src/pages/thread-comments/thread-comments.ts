...
export class ThreadCommentsPage {
  ...
  constructor(
    ...
    self.dataService.getThreadCommentsRef(self.threadKey).once(
      "value",
      function(snapshot) {
  ...
  vote(like: boolean, comment: IComment) {
    ...
    self.dataService
      .voteComment(comment.key, like, self.authService.getLoggedInUser().uid)
      .then(function() {
        self.dataService
          .getCommentsRef()
          .child(comment.key)
          ...
          .then(function(snapshot) {
            ...
            self.itemsService.setItem<IComment>(
              self.comments,
              c => c.key === comment.key,
              comment
            );
  ...
  addThreadToFavorites() {
    ...
    if (currentUser != null) {
      self.dataService
        .addThreadToFavorites(currentUser.uid, self.threadKey)
        .then(function() {
